...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('telecel_cash_analyzer.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# TELECEL THEME COLORS (Deep Red #a00030)
BACKGROUND_COLOR = "#1E1E2F"  # Dark background
SIDEBAR_COLOR = "#1b2944"     # Sidebar background
CONTENT_BG_COLOR = "#3a2a3a"  # Content area background (purple-ish)
TEXT_COLOR = "#FFFFFF"        # White text
BUTTON_COLOR = "#a00030"      # Telecel Deep Red
BUTTON_TEXT_COLOR = "#FFFFFF" # White text on buttons
HIGHLIGHT_COLOR = "#800020"   # Darker red on hover
...
class AnalysisThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Fixed the Series.append() error by using pd.concat()
    ...
    def generate_bar_chart(self, data, title, xlabel, ylabel, filename):
        ...
        plt.figure(figsize=(10, 6))
        data.plot(kind='bar', color='#a00030')  # Using Telecel red for bars
        plt.title(title)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.grid(True)
...
class TelecelCashAnalyzer(QMainWindow):
    def __init__(self, back_to_home_callback):
        super().__init__()
        self.back_to_home_callback = back_to_home_callback
        self.setWindowTitle("Telecel Cash Transaction Analyzer")
    ...
    def add_sidebar_logo(self):
        """Add Telecel logo to sidebar with matching background"""
        logo_frame = QFrame()
        ...
        logo_label = QLabel()
        logo_path = os.path.join("assets", "telecel_cash.png")
        if os.path.exists(logo_path):
            ...
            logo_label.setText("Telecel Cash")
    ...
    def create_home_page(self):
        ...
        title = QLabel("Telecel Cash Transaction Analyzer")
    ...
    def create_analysis_page(self):
        ...
        title = QLabel("Telecel Cash Analysis Report")
    ...
    def load_file(self):
        """Load Excel file with Telecel Cash format"""
        filename, _ = QFileDialog.getOpenFileName(
            self, "Open Telecel Cash File", "", "Excel Files (*.xlsx *.xls);;CSV Files (*.csv)"
        )
        ...
        if filename:
            try:
                ...
                # Check for required columns in Telecel format
                ...
                if missing_columns:
                    QMessageBox.critical(
                        self, 
                        "Error", 
                        f"File doesn't match Telecel Cash format.\nMissing columns: {', '.join(missing_columns)}"
                    )
                    self.df = None
                    return
                ...
                QMessageBox.information(self, "Success", "Telecel Cash file loaded successfully!")
    ...
    def export_report(self):
        """Export report to Word document with Telecel branding"""
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save Telecel Cash Report", "", "Word Documents (*.docx)"
        )
        ...
        try:
            ...
            # Add title and metadata with Telecel branding
            doc.add_heading('Telecel Cash Transaction Analysis Report', 0)
            ...
            QMessageBox.information(self, "Success", f"Telecel Cash report exported to {filename}")
...
if __name__ == "__main__":
    ...
    window = TelecelCashAnalyzer(lambda: None)  # Placeholder callback for standalone testing
    window.show()
    sys.exit(app.exec_())