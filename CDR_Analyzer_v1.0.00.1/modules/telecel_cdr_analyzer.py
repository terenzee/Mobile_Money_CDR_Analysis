...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('telecel_cdr_analyzer.log'),
...
# Theme Configuration
BACKGROUND_COLOR = "#1b2944"  # Dark blue background
SIDEBAR_BG_COLOR = "#1b2944"  # Sidebar background (same as main for now)
CONTENT_BG_COLOR = "#2a3a5a"  # Content area background
TEXT_COLOR = "#FFFFFF"        # White text
ACCENT_COLOR = "#800020"      # Telecel burgundy
ACCENT_TEXT = "#FFFFFF"       # White text on burgundy
HIGHLIGHT_COLOR = "#A00030"   # Slightly lighter burgundy for hover
...
class AnalysisThread(QThread):
    ...
    analysis_complete = pyqtSignal(list, dict)  # Now includes visualization paths and insights
    error_occurred = pyqtSignal(str)

    def __init__(self, df):
        super().__init__()
        self.df = df
        self.geolocator = Nominatim(user_agent="telecel_cdr_analyzer")
    ...
    def run(self):
        try:
            ...
            results = []
            total_steps = 15  # Increased for additional visualizations and insights
            current_step = 0
            ...
            # Calculate call durations (excluding SMS)
            ...
            # Generate map only after all analysis is complete
            self.generate_map(calls)
            self.visualizations['map'] = "Telecel_CDR_Map.html"
    ...
    def generate_map(self, calls):
        """Generate and save the Folium map"""
        try:
            calls = calls.dropna(subset=["Latitude", "Longitude"])
            if calls.empty:
...
class TelecelCDRAnalyzer(QMainWindow):
    def __init__(self, back_to_home_callback=None):
        ...
        self.filename = None
        self.df = None
        self.map_path = os.path.join(os.getcwd(), "Telecel_CDR_Map.html")
        self.doc_path = None
    ...
    def add_sidebar_logo(self):
        ...
        logo_frame.setStyleSheet(f"background-color: {SIDEBAR_BG_COLOR}; border: none;")
        logo_layout = QVBoxLayout(logo_frame)
        logo_layout.setAlignment(Qt.AlignCenter)
        logo_layout.setContentsMargins(5, 5, 5, 5)
        
        logo_label = QLabel()
        logo_path = os.path.join("assets", "telecel_icon.png")
    ...
    def back_to_home(self):
        ...
        if self.back_to_home_callback:
            ...
            self.close()  # Fallback if no callback provided
    ...
    def create_home_page(self):
        ...
        title = QLabel("Telecel Call Detail Records Analyzer")
    ...
    def show_visualizations(self):
        ...
        for viz_type, viz_path in self.visualizations.items():
            if viz_type == 'map':
                continue  # Skip map, handled separately
    ...
    def export_report(self):
        ...
        try:
            ...
            doc.add_heading('Telecel CDR Analysis Report', 0)
